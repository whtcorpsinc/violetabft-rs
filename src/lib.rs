...
## Creating a VioletaBFT node

You can use [`RawNode::new`](raw_node/struct.RawNode.html#method.new) to create the VioletaBFT node. To
create the VioletaBFT node, you need to provide a [`Storage`](storage/trait.Storage.html) component, and
...
// Finally, create our VioletaBFT node!
...
## Ticking the VioletaBFT node

Use a timer to tick the VioletaBFT node at regular intervals. See the following example using Rust
channel `recv_timeout` to drive the VioletaBFT node at least every 100ms, calling
[`tick()`](raw_node/struct.RawNode.html#method.tick) each time.

```rust
# use slog::{Drain, o};
# use violetabft::{Config, storage::MemStorage, raw_node::RawNode};
...
loop {
    ...
    if elapsed >= remaining_timeout {
        ...
        // We drive VioletaBFT every 100ms.
        node.tick();
    } else {
...
## Proposing to, and stepping the VioletaBFT node

Using the `propose` function you can drive the VioletaBFT node when the client sends a request to the
VioletaBFT server. You can call `propose` to add the request to the VioletaBFT log explicitly.
...
write flow is asynchronous in VioletaBFT, so the write log entry must be replicated to other followers,
...
You can call the `step` function when you receive the VioletaBFT messages from other nodes.

Here is a simple example to use `propose` and `step`:

```rust
# use violetabft::{Config, storage::MemStorage, raw_node::RawNode, evioletabftpb::Message};
# use std::{
...
loop {
    ...
    match rx.recv_timeout(remaining_timeout) {
        ...
        Ok(Msg::VioletaBFT(m)) => node.step(m).unwrap(),
        Err(RecvTimeoutError::Timeout) => (),
        Err(RecvTimeoutError::Disconnected) => unimplemented!(),
    }
...
propose the request ID to the VioletaBFT log. In your own practice, you can embed the ID in your request
and propose the encoded binary request data.

## Processing the `Ready` State

When your VioletaBFT node is ticked and running, VioletaBFT should enter a `Ready` state. You need to first use
`has_ready` to check whether VioletaBFT is ready. If yes, use the `ready` function to get a `Ready`
state:
...
// The VioletaBFT is ready, we can do something now.
...
1. Check whether `snapshot` is empty or not. If not empty, it means that the VioletaBFT node has received
a VioletaBFT snapshot from the leader and we must apply the snapshot:
...
entries but has not been committed yet, we must append the entries to the VioletaBFT log:
    ...
    if !ready.entries().is_empty() {
        // Append entries to the VioletaBFT log
...
We must persist the changed `HardState`:
    ...
    if let Some(hs) = ready.hs() {
        // VioletaBFT HardState changed, and we need to persist it.
...
messages to the leader after appending the VioletaBFT entries:

    ```rust
    # use slog::{Drain, o};
    # use violetabft::{Config, storage::MemStorage, raw_node::RawNode, StateRole};
    #
    # let config = Config { id: 1, ..Default::default() };
    # config.validate().unwrap();
    # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
    ...
    # }
    # let mut ready = node.ready();
    # let is_leader = node.violetabft.state == StateRole::Leader;
    #
    if !is_leader {
        // If not leader, the follower needs to reply the messages to
        // the leader after appending VioletaBFT entries.
...
need to update the applied index and resume `apply` later:
    ...
    ```rust
    # use slog::{Drain, o};
    # use violetabft::{Config, storage::MemStorage, raw_node::RawNode, evioletabftpb::EntryType};
    #
    # let config = Config { id: 1, ..Default::default() };
    # config.validate().unwrap();
    # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
    ...
    # let mut ready = node.ready();
    #
    # fn handle_conf_change(e:  violetabft::evioletabftpb::Entry) {
    # }
    #
    # fn handle_normal(e:  violetabft::evioletabftpb::Entry) {
    # }
    #
...
the membership of a peer group *dynamically, without downtime.* This VioletaBFT crate supports this via
**Joint Consensus**
([VioletaBFT paper, section 6](https://web.stanford.edu/~ouster/cgi-bin/papers/violetabft-atc14)).
...
```rust
use violetabft::{Config, storage::MemStorage, raw_node::RawNode, evioletabftpb::*};
...
pub use self::violetabft::{vote_resp_msg_type, VioletaBFT, SoftState, StateRole, INVALID_ID, INVALID_INDEX};
pub use self::violetabft_log::{VioletaBFTLog, NO_LIMIT};
...
pub use self::storage::{VioletaBFTState, Storage};
pub use self::util::majority;
pub use violetabft_proto::evioletabftpb;

pub mod prelude {
    //! A "prelude" for crates using the `violetabft` crate.
    //!
    ...
    //!
    //! ```
    //! use violetabft::prelude::*;
    //! ```
    //!
    //! The prelude may grow over time as additional items see ubiquitous use.

    pub use violetabft_proto::prelude::*;

    pub use crate::config::Config;
    pub use crate::violetabft::VioletaBFT;

    pub use crate::storage::{VioletaBFTState, Storage};
