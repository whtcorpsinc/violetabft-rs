...
use super::violetabft_log::VioletaBFTLog;
...
/// A constant represents invalid id of violetabft.
pub const INVALID_ID: u64 = 0;
/// A constant represents invalid index of violetabft log.
...
pub struct SoftState {
    ...
    pub violetabft_state: StateRole,
}

/// The core struct of violetabft consensus.
///
/// It's a helper struct to get around rust borrow checks.
#[derive(Getters)]
pub struct VioletaBFTCore<T: Storage> {
    /// The current election term.
    pub term: u64,

    /// Which peer this violetabft is voting for.
    pub vote: u64,
    ...
    /// The persistent log.
    pub violetabft_log: VioletaBFTLog<T>,
    ...
    ///
    /// If this is Some(id), we follow the procedure defined in violetabft thesis 3.10.
    ...
    // when violetabft changes its state to follower or candidate.
    randomized_election_timeout: usize,
    min_election_timeout: usize,
    max_election_timeout: usize,

    /// The logger for the violetabft structure.
...
/// A struct that represents the violetabft consensus itself. Stores details concerning the current
/// and possible state the system can take.
pub struct VioletaBFT<T: Storage> {
    prs: ProgressTracker,

    /// The list of messages.
    pub msgs: Vec<Message>,
    /// Internal violetabftCore.
    pub r: VioletaBFTCore<T>,
}

impl<T: Storage> Deref for VioletaBFT<T> {
    type Target = VioletaBFTCore<T>;

    #[inline]
    fn deref(&self) -> &VioletaBFTCore<T> {
        &self.r
    }
}

impl<T: Storage> DerefMut for VioletaBFT<T> {
    #[inline]
...
impl<T: Storage + Send> AssertSend for VioletaBFT<T> {}
...
impl<T: Storage> VioletaBFT<T> {
    /// Creates a new violetabft for use on the node.
    #[allow(clippy::new_ret_no_self)]
    pub fn new(c: &Config, store: T, logger: &Logger) -> Result<Self> {
        c.validate()?;
        let logger = logger.new(o!("violetabft_id" => c.id));
        let violetabft_state = store.initial_state()?;
        let conf_state = &violetabft_state.conf_state;
        let voters = &conf_state.voters;
        let learners = &conf_state.learners;

        let mut r = VioletaBFT {
            ...
            r: VioletaBFTCore {
                ...
                violetabft_log: VioletaBFTLog::new(store, logger.clone()),
        ...
        confchange::restore(&mut r.prs, r.r.violetabft_log.last_index(), conf_state)?;
        let new_cs = r.post_conf_change();
        if !violetabft_proto::conf_state_eq(&new_cs, conf_state) {
            fatal!(
                r.logger,
                "invalid restore: {:?} != {:?}",
                conf_state,
        ...
        if violetabft_state.hard_state != HardState::default() {
            r.load_state(&violetabft_state.hard_state);
        }
        if c.applied > 0 {
        ...
        info!(
            ...
            "newVioletaBFT";
            "term" => r.term,
            "commit" => r.violetabft_log.committed,
            "applied" => r.violetabft_log.applied,
            "last index" => r.violetabft_log.last_index(),
            "last term" => r.violetabft_log.last_term(),
    ...
    /// Creates a new violetabft for use on the node with the default logger.
    ///
    ...
    pub fn hard_state(&self) -> HardState {
        ...
        hs.term = self.term;
        hs.vote = self.vote;
        hs.commit = self.violetabft_log.committed;
        hs
    }

    /// Returns whether the current violetabft is in lease.
    ...
    /// it every time a violetabft state machine is initialized or a snapshot is applied.
    ...
    /// Checks whether the violetabft group is using group commit and consistent
    /// over group.
    ///
    ...
    pub fn check_group_commit_consistent(&mut self) -> Option<bool> {
        ...
        debug!(
            ...
            "committed" => self.violetabft_log.committed
        );
        Some(use_group_commit && index == self.violetabft_log.committed)
    }

    /// Checks if logs are committed to its term.
    ///
    /// The check is useful usually when violetabft is leader.
    pub fn commit_to_current_term(&self) -> bool {
        self.violetabft_log
            .term(self.violetabft_log.committed)
    ...
    pub fn apply_to_current_term(&self) -> bool {
        self.violetabft_log
            .term(self.violetabft_log.applied)
            .map_or(false, |t| t == self.term)
    }
}

impl<T: Storage> VioletaBFTCore<T> {
    ...
    fn prepare_send_snapshot(&mut self, m: &mut Message, pr: &mut Progress, to: u64) -> bool {
        ...
        let snapshot_r = self.violetabft_log.snapshot(pr.pending_request_snapshot);
        ...
        debug!(
            ...
            first_index = self.violetabft_log.first_index(),
            committed = self.violetabft_log.committed,
    ...
    fn prepare_send_entries(
        ...
        m.commit = self.violetabft_log.committed;
    ...
    fn try_batching(
        ...
        for msg in msgs {
            if msg.get_msg_type() == MessageType::MsgAppend && msg.to == to {
                ...
                msg.commit = self.violetabft_log.committed;
                is_batched = true;
                break;
            }
        }
    ...
    fn maybe_send_append(
        ...
        if pr.pending_request_snapshot != INVALID_INDEX {
            ...
            let ents = self.violetabft_log.entries(pr.next_idx, self.max_msg_size);
            if !allow_empty && ents.as_ref().ok().map_or(true, |e| e.is_empty()) {
                return false;
            }
            let term = self.violetabft_log.term(pr.next_idx - 1);
            match (term, ents) {
                (Ok(term), Ok(mut ents)) => {
    ...
    fn send_heartbeat(
        ...
        // Attach the commit as min(to.matched, self.violetabft_log.committed).
        ...
        let commit = cmp::min(pr.matched, self.violetabft_log.committed);
        m.commit = commit;
...
impl<T: Storage> VioletaBFT<T> {
    ...
    pub fn maybe_commit(&mut self) -> bool {
        ...
        if self.r.violetabft_log.maybe_commit(mci, self.r.term) {
            let (self_id, committed) = (self.id, self.violetabft_log.committed);
            self.mut_prs()
                .get_mut(self_id)
                .unwrap()
    ...
    /// Commit that the VioletaBFT peer has applied up to the given index.
    ///
    /// Registers the new applied index to the VioletaBFT log.
    ///
    /// # Hooks
    ///
    /// * Post: Checks to see if it's time to finalize a Joint Consensus state.
    pub fn commit_apply(&mut self, applied: u64) {
        let old_applied = self.violetabft_log.applied;
        #[allow(deprecated)]
        self.violetabft_log.applied_to(applied);
            ...
            && self.state == StateRole::Leader
        {
            ...
            self.pending_conf_index = self.violetabft_log.last_index();
    ...
    pub fn reset(&mut self, term: u64) {
        ...
        let last_index = self.violetabft_log.last_index();
        let committed = self.violetabft_log.committed;
        let self_id = self.id;
    ...
    pub fn append_entry(&mut self, es: &mut [Entry]) {
        let mut li = self.violetabft_log.last_index();
        for (i, e) in es.iter_mut().enumerate() {
            e.term = self.term;
            e.index = li + 1 + i as u64;
        }
        // use latest "last" index after truncate/append
        li = self.violetabft_log.append(es);
    ...
    // TODO: revoke pub when there is a better way to test.
    /// Makes this violetabft the leader.
    ///
    /// # Panics
    ///
    ...
    pub fn campaign(&mut self, campaign_type: &[u8]) {
        ...
        for id in self.prs.conf().voters().ids().iter() {
            ...
            m.term = term;
            m.index = self.violetabft_log.last_index();
            m.log_term = self.violetabft_log.last_term();
    ...
    fn log_broadcast_vote(&self, t: MessageType, ids: &[u64]) {
        info!(
            ...
            "type" => ?t,
            "term" => self.term,
            "log_term" => self.violetabft_log.last_term(),
            "log_index" => self.violetabft_log.last_index(),
            "to" => ?ids,
        );
    }

    /// Steps the violetabft along via a message. This should be called everytime your violetabft receives a
    /// message from a peer.
    pub fn step(&mut self, m: Message) -> Result<()> {
        ...
        if m.term == 0 {
                    ...
                    || m.get_msg_type() == MessageType::MsgAppend)
            {
                ...
                info!(
                    ...
                    self.id,
                    self.violetabft_log.last_term(),
                    self.violetabft_log.last_index(),
                    self.vote,
                    m.get_msg_type(),
                    m.from,
                    m.log_term,
        ...
        match m.get_msg_type() {
            ...
            MessageType::MsgRequestVote | MessageType::MsgRequestPreVote => {
                ...
                if can_vote
                    && self.violetabft_log.is_up_to_date(m.index, m.log_term)
                    && (m.index > self.violetabft_log.last_index() || self.priority <= m.priority)
                {
    ...
    fn hup(&mut self, transfer_leader: bool) {
        ...
        let first_index = match self.violetabft_log.unstable.maybe_first_index() {
            Some(idx) => idx,
            None => self.violetabft_log.applied + 1,
        };

        let ents = self
            .violetabft_log
            .slice(first_index, self.violetabft_log.committed + 1, None)
            .unwrap_or_else(|e| {
                fatal!(
                    self.logger,
                    "unexpected error getting unapplied entries [{}, {}): {:?}",
                    first_index,
                    self.violetabft_log.committed + 1,
                    e
                );
            });
    ...
    fn handle_append_response(&mut self, m: &Message) {
        ...
        if Some(m.from) == self.r.lead_transferee {
            let last_index = self.r.violetabft_log.last_index();
    ...
    fn handle_heartbeat_response(&mut self, m: &Message) {
        ...
        if pr.matched < self.r.violetabft_log.last_index() || pr.pending_request_snapshot != INVALID_INDEX
        {
    ...
    fn handle_transfer_leader(&mut self, m: &Message) {
        ...
        if pr.matched == self.r.violetabft_log.last_index() {
        ...
        match m.get_msg_type() {
            ...
            MessageType::MsgPropose => {
                ...
                for (i, e) in m.mut_entries().iter_mut().enumerate() {
                    ...
                    if reason.is_empty() {
                        self.pending_conf_index = self.violetabft_log.last_index() + i as u64 + 1;
                    } else {
                        info!(
                            self.logger,
                            ...
                            "applied" => self.violetabft_log.applied,
                        );
                        *e = Entry::default();
            ...
            MessageType::MsgReadIndex => {
                ...
                if self.prs().is_singleton() {
                    let read_index = self.violetabft_log.committed;
                ...
                match self.read_only.option {
                    ReadOnlyOption::Safe => {
                        ...
                        self.r
                            .read_only
                            .add_request(self.r.violetabft_log.committed, m, self.r.id);
    ...
    fn step_follower(&mut self, mut m: Message) -> Result<()> {
        match m.get_msg_type() {
            ...
            MessageType::MsgReadIndexResp => {
                ...
                self.violetabft_log.maybe_commit(m.index, m.term);
            }
            _ => {}
        }
        Ok(())
    ...
    pub fn handle_append_entries(&mut self, m: &Message) {
        ...
        if m.index < self.violetabft_log.committed {
            debug!(
                self.logger,
            ...
            to_send.to = m.from;
            to_send.index = self.violetabft_log.committed;
            to_send.commit = self.violetabft_log.committed;
            self.r.send(to_send, &mut self.msgs);
            return;
        }
            ...
            .maybe_append(m.index, m.log_term, m.commit, &m.entries)
        {
            ...
            debug!(
                ...
                from = m.from;
                "index" => m.index,
                "logterm" => ?self.violetabft_log.term(m.index),
            );
            to_send.index = m.index;
            to_send.reject = true;
            to_send.reject_hint = self.violetabft_log.last_index();
        }

        to_send.set_commit(self.violetabft_log.committed);
        self.r.send(to_send, &mut self.msgs);
    }
    ...
    pub fn handle_heartbeat(&mut self, mut m: Message) {
        self.violetabft_log.commit_to(m.commit);
        if self.pending_request_snapshot != INVALID_INDEX {
            self.send_request_snapshot();
            return;
        }
    ...
    fn handle_snapshot(&mut self, mut m: Message) {
        ...
        if self.restore(m.take_snapshot()) {
            info!(
                ...
                term = self.term,
                commit = self.violetabft_log.committed,
                snapshot_index = sindex,
                snapshot_term = sterm;
            );
            let mut to_send = Message::default();
            to_send.set_msg_type(MessageType::MsgAppendResponse);
            to_send.to = m.from;
            to_send.index = self.violetabft_log.last_index();
            self.r.send(to_send, &mut self.msgs);
        } else {
            info!(
                self.logger,
    ...
    pub fn restore(&mut self, snap: Snapshot) -> bool {
        if snap.get_metadata().index < self.violetabft_log.committed {
            return false;
        }
        ...
        if self.pending_request_snapshot == INVALID_INDEX
            && self.violetabft_log.match_term(meta.index, meta.term)
        {
            info!(
                self.logger,
                "fast-forwarded commit to snapshot";
                "commit" => self.violetabft_log.committed,
                "last_index" => self.violetabft_log.last_index(),
                "last_term" => self.violetabft_log.last_term(),
                "snapshot_index" => snap_index,
                "snapshot_term" => snap_term
            );
            self.violetabft_log.commit_to(meta.index);
            return false;
        }

        self.violetabft_log.restore(snap);
        let cs = self
            .r
            .violetabft_log
            .pending_snapshot()
        ...
        let cs = self
            ...
            .get_metadata()
            .get_conf_state();
        if !violetabft_proto::conf_state_eq(cs, &new_cs) {
    ...
    pub fn has_pending_conf(&self) -> bool {
        self.pending_conf_index > self.violetabft_log.applied
    }
    ...
    pub fn apply_conf_change(&mut self, cc: &ConfChangeV2) -> Result<ConfState> {
        ...
        self.prs
            .apply_conf(cfg, changes, self.violetabft_log.last_index());
    ...
    pub fn load_state(&mut self, hs: &HardState) {
        if hs.commit < self.violetabft_log.committed || hs.commit > self.violetabft_log.last_index() {
            fatal!(
                self.logger,
                "hs.commit {} is out of range [{}, {}]",
                hs.commit,
                self.violetabft_log.committed,
                self.violetabft_log.last_index()
            )
        }
        self.violetabft_log.committed = hs.commit;
        self.term = hs.term;
        self.vote = hs.vote;
    }
    ...
    // the view of the local violetabft state machine. Otherwise, it returns
    // false.
    ...
    fn send_request_snapshot(&mut self) {
        ...
        m.index = self.violetabft_log.committed;
        m.reject = true;
        m.reject_hint = self.violetabft_log.last_index();
        m.to = self.leader_id;
