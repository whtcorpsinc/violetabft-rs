...
use crate::evioletabftpb::{Entry, Snapshot};
...
/// VioletaBFT log implementation
pub struct VioletaBFTLog<T: Storage> {
...
impl<T> ToString for VioletaBFTLog<T>
where
    T: Storage,
{
...
impl<T: Storage> VioletaBFTLog<T> {
    /// Creates a new violetabft log with a given storage and tag.
    pub fn new(store: T, logger: Logger) -> VioletaBFTLog<T> {
    ...
    #[deprecated = "Call violetabft::commit_apply(idx) instead. Joint Consensus requires an on-apply hook to
    ...
    /// Grabs a slice of entries from the violetabft. Unlike a rust slice pointer, these are
...
mod test {
    ...
    use crate::violetabft_log::{self, VioletaBFTLog};
    use crate::storage::MemStorage;
    use protobuf::Message as PbMessage;

    fn new_entry(index: u64, term: u64) -> evioletabftpb::Entry {
        let mut e = evioletabftpb::Entry::default();
        e.term = term;
        e.index = index;
        e
    }

    fn new_snapshot(meta_index: u64, meta_term: u64) -> evioletabftpb::Snapshot {
        let mut meta = evioletabftpb::SnapshotMetadata::default();
        meta.index = meta_index;
        meta.term = meta_term;
        let mut snapshot = evioletabftpb::Snapshot::default();
    ...
    fn test_find_conflict() {
        ...
        for (i, &(ref ents, wconflict)) in tests.iter().enumerate() {
            ...
            let mut violetabft_log = VioletaBFTLog::new(store, l.clone());
            violetabft_log.append(&previous_ents);
            let gconflict = violetabft_log.find_conflict(ents);
    ...
    fn test_is_up_to_date() {
        let previous_ents = vec![new_entry(1, 1), new_entry(2, 2), new_entry(3, 3)];
        let store = MemStorage::new();
        let mut violetabft_log = VioletaBFTLog::new(store, default_logger());
        violetabft_log.append(&previous_ents);
        let tests = vec![
            // greater term, ignore lastIndex
            (violetabft_log.last_index() - 1, 4, true),
            (violetabft_log.last_index(), 4, true),
            (violetabft_log.last_index() + 1, 4, true),
            // smaller term, ignore lastIndex
            (violetabft_log.last_index() - 1, 2, false),
            (violetabft_log.last_index(), 2, false),
            (violetabft_log.last_index() + 1, 2, false),
            // equal term, lager lastIndex wins
            (violetabft_log.last_index() - 1, 3, false),
            (violetabft_log.last_index(), 3, true),
            (violetabft_log.last_index() + 1, 3, true),
        ];
        for (i, &(last_index, term, up_to_date)) in tests.iter().enumerate() {
            let g_up_to_date = violetabft_log.is_up_to_date(last_index, term);
    ...
    fn test_compaction_side_effects() {
        ...
        let mut violetabft_log = VioletaBFTLog::new(storage, default_logger());
        for i in unstable_index..last_index {
            violetabft_log.append(&[new_entry(i as u64 + 1, i as u64 + 1)]);
        }

        assert!(
            violetabft_log.maybe_commit(last_index, last_term),
            "maybe_commit return false"
        );
        let committed = violetabft_log.committed;
        #[allow(deprecated)]
        violetabft_log.applied_to(committed);
        let offset = 500u64;
        violetabft_log.store.wl().compact(offset).expect("compact failed");

        assert_eq!(last_index, violetabft_log.last_index());

        for j in offset..=violetabft_log.last_index() {
            assert_eq!(j, violetabft_log.term(j).expect(""));
            if !violetabft_log.match_term(j, j) {
                panic!("match_term({}) = false, want true", j);
            }
        }

        {
            let unstable_ents = violetabft_log.unstable_entries().expect("should have content.");
            assert_eq!(250, unstable_ents.len());
            assert_eq!(751, unstable_ents[0].index);
        }

        let mut prev = violetabft_log.last_index();
        violetabft_log.append(&[new_entry(prev + 1, prev + 1)]);
        assert_eq!(prev + 1, violetabft_log.last_index());

        prev = violetabft_log.last_index();
        let ents = violetabft_log.entries(prev, None).expect("unexpected error");
    ...
    fn test_term_with_unstable_snapshot() {
        ...
        store
            ...
            .expect("apply failed.");
        let mut violetabft_log = VioletaBFTLog::new(store, default_logger());
        violetabft_log.restore(new_snapshot(unstablesnapi, 1));
        ...
        for (i, &(index, w)) in tests.iter().enumerate() {
            let term = violetabft_log.term(index).expect("");
            if term != w {
    ...
    fn test_term() {
        ...
        let mut violetabft_log = VioletaBFTLog::new(store, default_logger());
        for i in 1..num {
            violetabft_log.append(&[new_entry(offset + i, i)]);
        }
    ...
    fn test_log_restore() {
        ...
        store
            ...
            .expect("apply failed.");
        let violetabft_log = VioletaBFTLog::new(store, default_logger());

        assert!(violetabft_log.all_entries().is_empty());
        assert_eq!(index + 1, violetabft_log.first_index());
        assert_eq!(index, violetabft_log.committed);
        assert_eq!(index + 1, violetabft_log.unstable.offset);
        let actual_term = violetabft_log.term(index).expect("");
    ...
    fn test_stable_to_with_snap() {
        ...
        for (i, &(stablei, stablet, ref new_ents, wunstable)) in tests.iter().enumerate() {
            ...
            store
                ...
                .expect("");
            let mut violetabft_log = VioletaBFTLog::new(store, l.clone());
            violetabft_log.append(new_ents);
            violetabft_log.stable_to(stablei, stablet);
            if violetabft_log.unstable.offset != wunstable {
                panic!(
                    "#{}: unstable = {}, want {}",
                    i, violetabft_log.unstable.offset, wunstable
                );
            }
        }
    }
    ...
    fn test_stable_to() {
        ...
        for (i, &(stablei, stablet, wunstable)) in tests.iter().enumerate() {
            let store = MemStorage::new();
            let mut violetabft_log = VioletaBFTLog::new(store, l.clone());
            violetabft_log.append(&[new_entry(1, 1), new_entry(2, 2)]);
            violetabft_log.stable_to(stablei, stablet);
            if violetabft_log.unstable.offset != wunstable {
                panic!(
                    "#{}: unstable = {}, want {}",
    ...
    fn test_unstable_ents() {
        ...
        for (i, &(unstable, ref wents)) in tests.iter().enumerate() {
            ...
            // append unstable entries to violetabftlog
            let mut violetabft_log = VioletaBFTLog::new(store, l.clone());
            violetabft_log.append(&previous_ents[(unstable - 1)..]);

            let ents = violetabft_log.unstable_entries().unwrap_or(&[]).to_vec();
            let l = ents.len();
            if l > 0 {
                violetabft_log.stable_to(ents[l - 1].index, ents[l - i].term);
            }
            if &ents != wents {
                panic!("#{}: unstableEnts = {:?}, want {:?}", i, ents, wents);
            }
            let w = previous_ents[previous_ents.len() - 1].index + 1;
            let g = violetabft_log.unstable.offset;
            if g != w {
    ...
    fn test_next_ents() {
        ...
        for (i, &(applied, ref expect_entries)) in tests.iter().enumerate() {
            ...
            let mut violetabft_log = VioletaBFTLog::new(store, l.clone());
            violetabft_log.append(&ents);
            violetabft_log.maybe_commit(5, 1);
            #[allow(deprecated)]
            violetabft_log.applied_to(applied);

            let next_entries = violetabft_log.next_entries();
    ...
    fn test_has_next_ents() {
        ...
        for (i, &(applied, has_next)) in tests.iter().enumerate() {
            ...
            let actual_has_next = violetabft_log.has_next_entries();
    ...
    fn test_slice() {
        ...
        let mut violetabft_log = VioletaBFTLog::new(store, default_logger());
        for i in (num / 2)..num {
            violetabft_log.append(&[new_entry(offset + i, offset + i)]);
        }

        let tests = vec![
            // test no limit
            (offset - 1, offset + 1, violetabft_log::NO_LIMIT, vec![], false),
            (offset, offset + 1, violetabft_log::NO_LIMIT, vec![], false),
            (
                half - 1,
                half + 1,
                violetabft_log::NO_LIMIT,
            ...
            (last, last + 1, violetabft_log::NO_LIMIT, vec![], true),
            // test limit
            (
                half - 1,
                half + 1,
        ...
        for (i, &(from, to, limit, ref w, wpanic)) in tests.iter().enumerate() {
            let res =
                panic::catch_unwind(AssertUnwindSafe(|| violetabft_log.slice(from, to, Some(limit))));
    ...
    fn test_log_maybe_append() {
            ...
            tests.iter().enumerate()
        {
            ...
            let mut violetabft_log = VioletaBFTLog::new(store, l.clone());
            violetabft_log.append(&previous_ents);
            violetabft_log.committed = commit;
            ...
            let gcommitted = violetabft_log.committed;
            if glasti != wlasti {
            ...
            if glasti.is_some() && ents_len != 0 {
                let (from, to) = (
                    violetabft_log.last_index() - ents_len + 1,
                    violetabft_log.last_index() + 1,
                );
                let gents = violetabft_log.slice(from, to, None).expect("");
                if &gents != ents {
    ...
    fn test_commit_to() {
        ...
        for (i, &(commit, wcommit, wpanic)) in tests.iter().enumerate() {
            ...
            let mut violetabft_log = VioletaBFTLog::new(store, l.clone());
            violetabft_log.append(&previous_ents);
            violetabft_log.committed = previous_commit;
            let has_panic =
                panic::catch_unwind(AssertUnwindSafe(|| violetabft_log.commit_to(commit))).is_err();
            if has_panic ^ wpanic {
                panic!("#{}: panic = {}, want {}", i, has_panic, wpanic)
            }
            if !has_panic && violetabft_log.committed != wcommit {
                let actual_committed = violetabft_log.committed;
    ...
    fn test_compaction() {
        ...
        for (i, &(index, ref compact, ref wleft, should_panic)) in tests.iter().enumerate() {
            ...
            let mut violetabft_log = VioletaBFTLog::new(store, l.clone());
            violetabft_log.maybe_commit(index - 1, 0);
            let committed = violetabft_log.committed;
            #[allow(deprecated)]
            violetabft_log.applied_to(committed);
            ...
            for (j, idx) in compact.iter().enumerate() {
                let res =
                    panic::catch_unwind(AssertUnwindSafe(|| violetabft_log.store.wl().compact(*idx)));
                if !(should_panic ^ res.is_ok()) {
                    panic!("#{}: should_panic: {}, but got: {:?}", i, should_panic, res);
                }
                if !should_panic {
                    let l = violetabft_log.all_entries().len();
                    if l != wleft[j] {
    ...
    fn test_is_outofbounds() {
        ...
        let mut violetabft_log = VioletaBFTLog::new(store, default_logger());
        for i in 1u64..=num {
            violetabft_log.append(&[new_entry(i + offset, 0)]);
        }
        let first = offset + 1;
        let tests = vec![
        ...
        for (i, &(lo, hi, wpanic, w_err_compacted)) in tests.iter().enumerate() {
            let res =
                panic::catch_unwind(AssertUnwindSafe(|| violetabft_log.must_check_outofbounds(lo, hi)));
            if res.is_err() ^ wpanic {
                panic!(
                    "#{}: panic = {}, want {}: {:?}",
                    i,
