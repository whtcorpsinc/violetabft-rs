...
use violetabft::{default_logger, Error, ProgressState, INVALID_INDEX};
...
fn test_snapshot_with_min_term() {
    ...
    let do_test = |pre_vote: bool| {
        ...
        s.wl()
            ...
            .unwrap();
        let n1 = new_test_violetabft_with_prevote(1, vec![1, 2], 10, 1, s, pre_vote, &l);
        let n2 = new_test_violetabft_with_prevote(2, vec![], 10, 1, new_storage(), pre_vote, &l);
        ...
        assert_eq!(nt.peers[&2].violetabft_log.first_index(), 2);
        assert_eq!(nt.peers[&2].violetabft_log.last_index(), 2);
    };
    do_test(true);
    do_test(false);
}
...
fn test_request_snapshot() {
    ...
    // VioletaBFT can not step request snapshot if there is no leader.
    assert_eq!(
        sm.violetabft
            .as_mut()
            .unwrap()
    ...
    // VioletaBFT can not step request snapshot if itself is a leader.
    assert_eq!(
        sm.violetabft
            .as_mut()
            .unwrap()
    ...
    let request_snapshot_idx = sm.violetabft_log.committed;
    let mut m = new_message(2, 1, MessageType::MsgAppendResponse, 0);
    m.index = 11;
    m.reject = true;
