...
use violetabft::storage::MemStorage;
use violetabft::*;
use slog::Logger;

use crate::integration_cases::test_violetabft_paper::commit_noop_entry;
use crate::test_util::*;

type HashSet<K> = std::collections::HashSet<K, std::hash::BuildHasherDefault<fxhash::FxHasher>>;

fn read_messages<T: Storage>(violetabft: &mut VioletaBFT<T>) -> Vec<Message> {
    violetabft.msgs.drain(..).collect()
}
...
fn ents_with_config(
    ...
    let mut violetabft = new_test_violetabft_with_prevote(id, peers, 5, 1, store, pre_vote, l);
    violetabft.reset(terms[terms.len() - 1]);
    violetabft
}

fn assert_violetabft_log(
    prefix: &str,
    violetabft_log: &VioletaBFTLog<MemStorage>,
    (committed, applied, last): (u64, u64, u64),
) {
    assert_eq!(
        violetabft_log.committed, committed,
        "{}committed = {}, want = {}",
        prefix, violetabft_log.committed, committed
    );
    assert_eq!(
        violetabft_log.applied, applied,
        "{}applied = {}, want = {}",
        prefix, violetabft_log.applied, applied
    );
    assert_eq!(
        violetabft_log.last_index(),
        last,
        "{}last_index = {}, want = {}",
        prefix,
        violetabft_log.last_index(),
...
// voted_with_config creates a violetabft state machine with vote and term set
...
fn voted_with_config(
    ...
    let mut violetabft = new_test_violetabft_with_prevote(id, peers, 5, 1, store, pre_vote, l);
    violetabft.reset(term);
    violetabft
}

// Persist committed index and fetch next entries.
fn next_ents(r: &mut VioletaBFT<MemStorage>, s: &MemStorage) -> Vec<Entry> {
    if let Some(entries) = r.violetabft_log.unstable_entries() {
        s.wl().append(entries).expect("");
    }
    let (last_idx, last_term) = (r.violetabft_log.last_index(), r.violetabft_log.last_term());
    r.violetabft_log.stable_to(last_idx, last_term);
    let ents = r.violetabft_log.next_entries();
    r.commit_apply(r.violetabft_log.committed);
...
fn test_progress_committed_index() {
    ...
    assert_violetabft_log("#1: ", &nt.peers[&1].violetabft_log, (1, 0, 1));
    assert_violetabft_log("#2: ", &nt.peers[&2].violetabft_log, (1, 0, 1));
    assert_violetabft_log("#3: ", &nt.peers[&3].violetabft_log, (1, 0, 1));
    ...
    assert_violetabft_log("#1: ", &nt.peers[&1].violetabft_log, (3, 0, 3));
    assert_violetabft_log("#2: ", &nt.peers[&2].violetabft_log, (3, 0, 3));
    assert_violetabft_log("#3: ", &nt.peers[&3].violetabft_log, (1, 0, 1));
    ...
    assert_violetabft_log("#1: ", &nt.peers[&1].violetabft_log, (3, 0, 3));
    assert_violetabft_log("#2: ", &nt.peers[&2].violetabft_log, (3, 0, 3));
    assert_violetabft_log("#3: ", &nt.peers[&3].violetabft_log, (3, 0, 3));
    ...
    assert_violetabft_log("#1: ", &nt.peers[&1].violetabft_log, (4, 0, 4));
    assert_violetabft_log("#2: ", &nt.peers[&2].violetabft_log, (4, 0, 4));
    assert_violetabft_log("#3: ", &nt.peers[&3].violetabft_log, (4, 0, 4));
    ...
    assert_violetabft_log("#1: ", &nt.peers[&1].violetabft_log, (8, 0, 8));
    assert_violetabft_log("#2: ", &nt.peers[&2].violetabft_log, (8, 0, 8));
    assert_violetabft_log("#3: ", &nt.peers[&3].violetabft_log, (8, 0, 8));
...
fn test_progress_leader() {
    let l = default_logger();
    let mut violetabft = new_test_violetabft(1, vec![1, 2], 5, 1, new_storage(), &l);
    violetabft.become_candidate();
    violetabft.become_leader();
    violetabft.mut_prs().get_mut(2).unwrap().become_replicate();

    let prop_msg = new_message(1, 1, MessageType::MsgPropose, 1);
    for i in 0..5 {
        assert_eq!(
            violetabft.mut_prs().get_mut(1).unwrap().state,
            ProgressState::Replicate
        );

        let matched = violetabft.mut_prs().get_mut(1).unwrap().matched;
        let next_idx = violetabft.mut_prs().get_mut(1).unwrap().next_idx;
        assert_eq!(matched, i + 1);
        assert_eq!(next_idx, matched + 1);

        assert!(violetabft.step(prop_msg.clone()).is_ok());
    }
}

// test_progress_resume_by_heartbeat_resp ensures violetabft.heartbeat reset progress.paused by
// heartbeat response.
#[test]
fn test_progress_resume_by_heartbeat_resp() {
    ...
    violetabft.become_candidate();
    violetabft.become_leader();
    violetabft.mut_prs().get_mut(2).unwrap().paused = true;

    violetabft.step(new_message(1, 1, MessageType::MsgBeat, 0))
        .expect("");
    assert!(violetabft.prs().get(2).unwrap().paused);

    violetabft.mut_prs().get_mut(2).unwrap().become_replicate();
    violetabft.step(new_message(2, 1, MessageType::MsgHeartbeatResponse, 0))
        .expect("");
    assert!(!violetabft.prs().get(2).unwrap().paused);
}
...
fn test_progress_paused() {
    ...
    violetabft.step(m.clone()).expect("");
    violetabft.step(m.clone()).expect("");
    violetabft.step(m).expect("");
    let ms = read_messages(&mut violetabft);
    assert_eq!(ms.len(), 1);
}
...
fn test_progress_flow_control() {
    ...
    let mut r = new_test_violetabft_with_config(&cfg, s, &l);
    r.become_candidate();
    r.become_leader();
...
fn test_leader_election_with_config(pre_vote: bool, l: &Logger) {
    ...
    for (i, &mut (ref mut network, state, term)) in tests.iter_mut().enumerate() {
        ...
        network.send(vec![m]);
        let violetabft = &network.peers[&1];
        ...
        if violetabft.state != exp_state {
            panic!("#{}: state = {:?}, want {:?}", i, violetabft.state, exp_state);
        }
        if violetabft.term != exp_term {
            panic!("#{}: term = {}, want {}", i, violetabft.term, exp_term)
        }
    }
}
...
fn test_leader_election_overwrite_newer_logs_with_config(pre_vote: bool, l: &Logger) {
    ...
    for (id, sm) in &network.peers {
        let entries = sm.violetabft_log.all_entries();
        assert_eq!(
            entries.len(),
            2,
...
fn test_vote_from_any_state_for_type(vt: MessageType, l: &Logger) {
    ...
    for state in all_states {
        let mut r = new_test_violetabft(1, vec![1, 2, 3], 10, 1, new_storage(), &l);
        r.term = 1;
        match state {
            StateRole::Follower => {
                let term = r.term;
...
fn test_log_replication() {
    ...
    for (i, &mut (ref mut network, ref msgs, wcommitted)) in tests.iter_mut().enumerate() {
        ...
        for (j, x) in &mut network.peers {
            if x.violetabft_log.committed != wcommitted {
                panic!(
                    "#{}.{}: committed = {}, want {}",
                    i, j, x.violetabft_log.committed, wcommitted
                );
            }
...
fn test_single_node_commit() {
    ...
    tt.send(vec![new_message(1, 1, MessageType::MsgPropose, 1)]);
    assert_eq!(tt.peers[&1].violetabft_log.committed, 3);
}
...
fn test_cannot_commit_without_new_term_entry() {
    ...
    assert_eq!(tt.peers[&1].violetabft_log.committed, 1);
    ...
    assert_eq!(tt.peers[&2].violetabft_log.committed, 1);
    ...
    assert_eq!(tt.peers[&2].violetabft_log.committed, 5);
}
...
fn test_commit_without_new_term_entry() {
    ...
    assert_eq!(tt.peers[&1].violetabft_log.committed, 4);
}

#[test]
fn test_dueling_candidates() {
    let l = default_logger();
    let a = new_test_violetabft(1, vec![1, 2, 3], 10, 1, new_storage(), &l);
    let b = new_test_violetabft(2, vec![1, 2, 3], 10, 1, new_storage(), &l);
    let c = new_test_violetabft(3, vec![1, 2, 3], 10, 1, new_storage(), &l);
    ...
    for (i, &(state, term, violetabft_log)) in tests.iter().enumerate() {
        let id = i as u64 + 1;
        ...
        let prefix = format!("#{}: ", i);
        assert_violetabft_log(&prefix, &nt.peers[&id].violetabft_log, violetabft_log);
    }
}
...
fn test_dueling_pre_candidates() {
    let l = default_logger();
    let a = new_test_violetabft_with_prevote(1, vec![1, 2, 3], 10, 1, new_storage(), true, &l);
    let b = new_test_violetabft_with_prevote(2, vec![1, 2, 3], 10, 1, new_storage(), true, &l);
    let c = new_test_violetabft_with_prevote(3, vec![1, 2, 3], 10, 1, new_storage(), true, &l);
    ...
    for (i, &(id, state, term, violetabft_log)) in tests.iter().enumerate() {
        if nt.peers[&id].state != state {
            panic!(
                "#{}: state = {:?}, want {:?}",
                i, nt.peers[&id].state, state
...
fn test_candidate_concede() {
    ...
    for p in tt.peers.values() {
        assert_eq!(p.violetabft_log.committed, 2); // All violetabft logs are committed.
        assert_eq!(p.violetabft_log.applied, 0); // VioletaBFT logs are based on a snapshot with index 1.
        assert_eq!(p.violetabft_log.last_index(), 2);
    }
}
...
fn test_old_messages() {
    ...
    for p in tt.peers.values() {
        let violetabft = p.violetabft.as_ref().unwrap();
        assert_eq!(violetabft.violetabft_log.committed, 4);
        assert_eq!(violetabft.violetabft_log.applied, 0);
        assert_eq!(violetabft.violetabft_log.last_index(), 4);
    }
}
...
fn test_proposal() {
    ...
    for (j, (mut nw, success)) in tests.drain(..).enumerate() {
        ...
        for p in nw.peers.values() {
            if let Some(ref violetabft) = p.violetabft {
                let prefix = format!("#{}: ", j);
                assert_violetabft_log(&prefix, &violetabft.violetabft_log, want_log);
            }
        }
...
fn test_proposal_by_proxy() {
    ...
    for (j, tt) in tests.iter_mut().enumerate() {
        ...
        for p in tt.peers.values() {
            if p.violetabft.is_none() {
                continue;
            }
            if let Some(ref violetabft) = p.violetabft {
                let prefix = format!("#{}: ", j);
                assert_violetabft_log(&prefix, &violetabft.violetabft_log, (2, 0, 2));
            }
        }
...
fn test_commit() {
    ...
    for (i, (matches, logs, sm_term, w)) in tests.drain(..).enumerate() {
        ...
        let mut sm = new_test_violetabft_with_config(&cfg, store, &l);
        ...
        sm.maybe_commit();
        if sm.violetabft_log.committed != w {
            panic!("#{}: committed = {}, want {}", i, sm.violetabft_log.committed, w);
        }
    }
}
...
fn test_pass_election_timeout() {
    ...
    for (i, &(elapse, wprobability, round)) in tests.iter().enumerate() {
        let mut sm = new_test_violetabft(1, vec![1], 10, 1, new_storage(), &l);
...
fn test_handle_msg_append() {
    ...
    for (j, (m, w_index, w_commit, w_reject)) in tests.drain(..).enumerate() {
        let mut sm = new_test_violetabft_with_logs(
            1,
            vec![1],
            10,
            1,
        ...
        if sm.violetabft_log.last_index() != w_index {
            panic!(
                "#{}: last_index = {}, want {}",
                j,
                sm.violetabft_log.last_index(),
                w_index
            );
        }
        if sm.violetabft_log.committed != w_commit {
            panic!(
                "#{}: committed = {}, want {}",
                j, sm.violetabft_log.committed, w_commit
            );
        }
...
fn test_handle_heartbeat() {
    ...
    for (i, (m, w_commit)) in tests.drain(..).enumerate() {
        ...
        sm.violetabft_log.commit_to(commit);
        sm.handle_heartbeat(m);
        if sm.violetabft_log.committed != w_commit {
            panic!(
                "#{}: committed = {}, want = {}",
                i, sm.violetabft_log.committed, w_commit
            );
        }
        let m = sm.read_messages();
        if m.len() != 1 {
...
fn test_handle_heartbeat_resp() {
    ...
    store
        ...
        .append(&[empty_entry(1, 1), empty_entry(2, 2), empty_entry(3, 3)])
        .unwrap();
    let mut sm = new_test_violetabft(1, vec![1, 2], 5, 1, store, &l);
    sm.become_candidate();
    sm.become_leader();
    let last_index = sm.violetabft_log.last_index();
    sm.violetabft_log.commit_to(last_index);
...
// test_violetabft_frees_read_only_mem ensures violetabft will free read request from
// ReadOnly read_index_queue and pending_read_index map.
// related issue: https://github.com/coreos/etcd/issues/7571
#[test]
fn test_violetabft_frees_read_only_mem() {
    let l = default_logger();
    let mut sm = new_test_violetabft(1, vec![1, 2], 5, 1, new_storage(), &l);
    sm.become_candidate();
    sm.become_leader();
    let last_index = sm.violetabft_log.last_index();
    sm.violetabft_log.commit_to(last_index);
    ...
    // more info: violetabft dissertation 6.4, step 2.
    ...
    // more info: violetabft dissertation 6.4, step 3.
...
fn test_msg_append_response_wait_reset() {
    let l = default_logger();
    let mut sm = new_test_violetabft(1, vec![1, 2, 3], 5, 1, new_storage(), &l);
    sm.become_candidate();
    sm.become_leader();
    ...
    m.index = 1;
    sm.step(m).expect("");
    assert_eq!(sm.violetabft_log.committed, 1);
...
fn test_recv_msg_request_vote_for_type(msg_type: MessageType, l: &Logger) {
    ...
    for (j, (state, index, log_term, vote_for, w_reject)) in tests.drain(..).enumerate() {
        ...
        let mut sm = new_test_violetabft(1, vec![1], 10, 1, store, &l);
        sm.state = state;
        sm.vote = vote_for;
        ...
        m.index = index;
        m.log_term = log_term;
        // violetabft.Term is greater than or equal to violetabft.violetabftLog.lastTerm. In this
        // test we're only testing MsgVote responses when the campaigning node
        // has a different violetabft log compared to the recipient node.
        // Additionally we're verifying behaviour when the recipient node has
        // already given out its vote for its current term. We're not testing
        // what the recipient node does when receiving a message with a
        // different term number, so we simply initialize both term numbers to
        // be the same.
        let term = cmp::max(sm.violetabft_log.last_term(), log_term);
        m.term = term;
        sm.term = term;
        sm.step(m).expect("");
...
fn test_state_transition() {
    ...
    for (i, (from, to, wallow, wterm, wlead)) in tests.drain(..).enumerate() {
        let sm: &mut VioletaBFT<MemStorage> = &mut new_test_violetabft(1, vec![1], 10, 1, new_storage(), &l);
        sm.state = from;
...
fn test_all_server_stepdown() {
    ...
    for (i, (state, wstate, wterm, windex, entries)) in tests.drain(..).enumerate() {
        let mut sm = new_test_violetabft(1, vec![1, 2, 3], 10, 1, new_storage(), &l);
        match state {
        ...
        for (j, &msg_type) in tmsg_types.iter().enumerate() {
            ...
            if sm.violetabft_log.last_index() != windex {
                panic!(
                    "{}.{} index = {}, want {}",
                    i,
                    j,
            ...
            let entry_count = sm.violetabft_log.all_entries().len() as u64;
...
fn test_leader_superseding_with_check_quorum() {
    let l = default_logger();
    let mut a = new_test_violetabft(1, vec![1, 2, 3], 10, 1, new_storage(), &l);
    let mut b = new_test_violetabft(2, vec![1, 2, 3], 10, 1, new_storage(), &l);
    let mut c = new_test_violetabft(3, vec![1, 2, 3], 10, 1, new_storage(), &l);
...
fn test_non_promotable_voter_with_check_quorum() {
    let l = default_logger();
    let mut a = new_test_violetabft(1, vec![1, 2], 10, 1, new_storage(), &l);
    let mut b = new_test_violetabft(2, vec![1], 10, 1, new_storage(), &l);
...
fn test_disruptive_follower() {
    let l = default_logger();
    let mut n1 = new_test_violetabft(1, vec![1, 2, 3], 10, 1, new_storage(), &l);
    let mut n2 = new_test_violetabft(2, vec![1, 2, 3], 10, 1, new_storage(), &l);
    let mut n3 = new_test_violetabft(3, vec![1, 2, 3], 10, 1, new_storage(), &l);
...
fn test_disruptive_follower_pre_vote() {
    let l = default_logger();
    let mut n1 = new_test_violetabft_with_prevote(1, vec![1, 2, 3], 10, 1, new_storage(), true, &l);
    let mut n2 = new_test_violetabft_with_prevote(2, vec![1, 2, 3], 10, 1, new_storage(), true, &l);
    let mut n3 = new_test_violetabft_with_prevote(3, vec![1, 2, 3], 10, 1, new_storage(), true, &l);
...
fn test_read_only_with_learner() {
    let l = default_logger();
    let a = new_test_learner_violetabft(1, vec![1], vec![2], 10, 1, new_storage(), &l);
    let b = new_test_learner_violetabft(2, vec![1], vec![2], 10, 1, new_storage(), &l);
...
fn test_read_only_for_new_leader() {
    ...
    for (id, committed, applied, compact_index) in node_configs {
        ...
        let i = new_test_violetabft_with_config(&cfg, storage, &l);
        peers.push(Some(i));
    }
    ...
    nt.send(vec![new_message(1, 1, MessageType::MsgPropose, 1)]);
    assert_eq!(nt.peers[&1].violetabft_log.committed, 4);
    assert_eq!(
        nt.peers[&1]
            .violetabft_log
            .term(nt.peers[&1].violetabft_log.committed)
            .unwrap_or(0),
        nt.peers[&1].term
    );
...
fn test_advance_commit_index_by_read_index_response() {
    ...
    assert_eq!(tt.peers[&2].violetabft_log.committed, 3);
}
...
fn test_leader_append_response() {
    ...
    {
        // Initial violetabft logs: last index = 3, committed = 1.
        let store = MemStorage::new_with_conf_state((vec![1, 2, 3], vec![]));
        let ents = &[empty_entry(0, 1), empty_entry(1, 2)];
        store.wl().append(ents).unwrap();
        let mut sm = new_test_violetabft(1, vec![1, 2, 3], 10, 1, store, &l);
...
fn test_bcast_beat() {
    ...
    want_commit_map.insert(
        2,
        cmp::min(sm.violetabft_log.committed, sm.prs().get(2).unwrap().matched),
    );
    want_commit_map.insert(
        3,
        cmp::min(sm.violetabft_log.committed, sm.prs().get(3).unwrap().matched),
    );
...
fn test_leader_increase_next() {
    ...
    for (i, (state, next_idx, wnext)) in tests.drain(..).enumerate() {
        let mut sm = new_test_violetabft(1, vec![1, 2], 10, 1, new_storage(), &l);
        sm.violetabft_log.append(&previous_ents);
        sm.become_candidate();
        sm.become_leader();
...
fn test_send_append_for_progress_probe() {
    let l = default_logger();
    let mut r = new_test_violetabft(1, vec![1, 2], 10, 1, new_storage(), &l);
    r.become_candidate();
    r.become_leader();
    r.read_messages();
    r.mut_prs().get_mut(2).unwrap().become_probe();
    ...
    for i in 0..3 {
        if i == 0 {
            // we expect that violetabft will only send out one msgAPP on the first
...
fn test_recv_msg_unreachable() {
    ...
    let mut r = new_test_violetabft(1, vec![1, 2], 10, 1, s, &l);
    r.become_candidate();
    r.become_leader();
    r.read_messages();
...
fn test_restore() {
    ...
    assert_eq!(sm.violetabft_log.last_index(), s.get_metadata().index);
    assert_eq!(
        sm.violetabft_log.term(s.get_metadata().index).unwrap(),
        s.get_metadata().term
    );
    assert_iter_eq!(
...
fn test_restore_ignore_snapshot() {
    ...
    // ingore snapshot
    assert!(!sm.restore(s.clone()));
    assert_eq!(sm.violetabft_log.committed, commit);

    // ignore snapshot and fast forward commit
    s.mut_metadata().index = commit + 1;
    assert!(!sm.restore(s));
    assert_eq!(sm.violetabft_log.committed, commit + 1);
}
...
fn test_provide_snap() {
    ...
    sm.mut_prs().get_mut(2).unwrap().next_idx = sm.violetabft_log.first_index();
...
fn test_ignore_providing_snapshot() {
    ...
    sm.mut_prs().get_mut(2).unwrap().next_idx = sm.violetabft_log.first_index() - 1;
...
fn test_restore_from_snap_msg() {
    let l = default_logger();
    let s = new_snapshot(11, 11, vec![1, 2]); // magic number
    let mut sm = new_test_violetabft(2, vec![1, 2], 10, 1, new_storage(), &l);
...
fn test_slow_node_restore() {
    ...
    nt.storage[&1]
        .wl()
        .commit_to(nt.peers[&1].violetabft_log.applied)
        .unwrap();
    nt.storage[&1]
        .wl()
        .compact(nt.peers[&1].violetabft_log.applied)
        .unwrap();
    ...
    nt.send(vec![new_message(1, 1, MessageType::MsgPropose, 1)]);
    assert_eq!(
        nt.peers[&3].violetabft_log.committed,
        nt.peers[&1].violetabft_log.committed
    );
}

// test_step_config tests that when violetabft step msgProp in EntryConfChange type,
// it appends the entry to log and sets pendingConf to be true.
#[test]
fn test_step_config() {
    let l = default_logger();
    // a violetabft that cannot make progress
    let mut r = new_test_violetabft(1, vec![1, 2], 10, 1, new_storage(), &l);
    r.become_candidate();
    r.become_leader();
    let index = r.violetabft_log.last_index();
    let mut m = new_message(1, 1, MessageType::MsgPropose, 0);
    let mut e = Entry::default();
    e.set_entry_type(EntryType::EntryConfChange);
    m.mut_entries().push(e);
    r.step(m).expect("");
    assert_eq!(r.violetabft_log.last_index(), index + 1);
}

// test_step_ignore_config tests that if violetabft step the second msgProp in
...
fn test_step_ignore_config() {
    ...
    let wents = vec![we];
    let entries = r.violetabft_log.entries(index + 1, None).expect("");
...
fn test_add_node() -> Result<()> {
    let l = default_logger();
    let mut r = new_test_violetabft(1, vec![1], 10, 1, new_storage(), &l);
...
fn test_remove_node_itself() -> Result<()> {
    let l = default_logger();
    let mut n1 = new_test_learner_violetabft(1, vec![1], vec![2], 10, 1, new_storage(), &l);
...
fn test_promotable() {
    ...
    for (i, (peers, wp)) in tests.drain(..).enumerate() {
        let r = new_test_violetabft(id, peers, 5, 1, new_storage(), &l);
        if r.promotable() != wp {
...
fn test_violetabft_nodes() {
    ...
    for (i, (ids, wids)) in tests.drain(..).enumerate() {
        let r = new_test_violetabft(1, ids, 10, 1, new_storage(), &l);
...
fn test_campaign_while_leader_with_pre_vote(pre_vote: bool, l: &Logger) {
    let mut r = new_test_violetabft_with_prevote(1, vec![1], 5, 1, new_storage(), pre_vote, l);
    assert_eq!(r.state, StateRole::Follower);
...
fn test_commit_after_remove_node() -> Result<()> {
    ...
    let s = new_storage();
    let mut r = new_test_violetabft(1, vec![1, 2], 5, 1, s.clone(), &l);
    r.become_candidate();
    r.become_leader();
    ...
    let cc_index = r.violetabft_log.last_index();
...
fn test_leader_transfer_to_learner() {
    ...
    let leader = new_test_violetabft_with_config(&c, s, &l);

    let s = MemStorage::new_with_conf_state((vec![1], vec![2]));
    let c = new_test_config(2, 10, 1);
    let learner = new_test_violetabft_with_config(&c, s, &l);
...
fn check_leader_transfer_state(r: &VioletaBFT<MemStorage>, state: StateRole, lead: u64) {
...
fn test_transfer_non_member() {
    let l = default_logger();
    let mut violetabft = new_test_violetabft(1, vec![2, 3, 4], 5, 1, new_storage(), &l);
    violetabft.step(new_message(2, 1, MessageType::MsgTimeoutNow, 0))
        .expect("");

    violetabft.step(new_message(2, 1, MessageType::MsgRequestVoteResponse, 0))
        .expect("");
    violetabft.step(new_message(3, 1, MessageType::MsgRequestVoteResponse, 0))
        .expect("");
    assert_eq!(violetabft.state, StateRole::Follower);
}
...
pub fn new_test_learner_violetabft(
    id: u64,
    peers: Vec<u64>,
    learners: Vec<u64>,
    election: usize,
    ...
    if storage.initial_state().unwrap().initialized() && peers.is_empty() {
        panic!("new_test_violetabft with empty peers on initialized store");
    }
    ...
    new_test_violetabft_with_config(&cfg, storage, logger)
}
...
fn test_learner_election_timeout() {
    ...
    let mut n2 = new_test_learner_violetabft(2, vec![1], vec![2], 10, 1, new_storage(), &l);
    n2.become_follower(1, INVALID_ID);
...
fn test_learner_log_replication() {
    let l = default_logger();
    let n1 = new_test_learner_violetabft(1, vec![1], vec![2], 10, 1, new_storage(), &l);
    let n2 = new_test_learner_violetabft(2, vec![1], vec![2], 10, 1, new_storage(), &l);
    let mut network = Network::new(vec![Some(n1), Some(n2)], &l);
    ...
    let next_committed = network.peers[&1].violetabft_log.committed + 1;

    let msg = new_message(1, 1, MessageType::MsgPropose, 1);
    network.send(vec![msg]);

    assert_eq!(network.peers[&1].violetabft_log.committed, next_committed);
    assert_eq!(network.peers[&2].violetabft_log.committed, next_committed);
    ...
    let matched = network
        ...
        .unwrap()
        .matched;
    assert_eq!(matched, network.peers[&2].violetabft_log.committed);
}
...
fn test_restore_with_learner() {
    ...
    let mut sm = new_test_learner_violetabft(3, vec![1, 2], vec![3], 10, 1, new_storage(), &l);
    assert!(!sm.promotable());
    assert!(sm.restore(s.clone()));
    assert_eq!(sm.violetabft_log.last_index(), 11);
    assert_eq!(sm.violetabft_log.term(11).unwrap(), 11);
...
fn test_restore_depromote_voter() {
    ...
    let mut sm = new_test_violetabft(3, vec![1, 2, 3], 10, 1, new_storage(), &l);
    assert!(sm.promotable());
    assert!(sm.restore(s));
}
...
fn test_restore_learner() {
    ...
    let mut sm = new_test_violetabft(3, vec![], 10, 1, new_storage(), &l);
...
fn test_learner_receive_snapshot() {
    ...
    n1.restore(s);
    let committed = n1.violetabft_log.committed;
    ...
    let n1_committed = network.peers[&1].violetabft_log.committed;
    let n2_committed = network.peers[&2].violetabft_log.committed;
...
fn test_add_learner() -> Result<()> {
    let l = default_logger();
    let mut n1 = new_test_violetabft(1, vec![1], 10, 1, new_storage(), &l);
...
fn new_prevote_migration_cluster(l: &Logger) -> Network {
    ...
    let mut n1 = new_test_violetabft_with_prevote(1, vec![1, 2, 3], 10, 1, new_storage(), true, &l);
    let mut n2 = new_test_violetabft_with_prevote(2, vec![1, 2, 3], 10, 1, new_storage(), true, &l);
    let mut n3 = new_test_violetabft_with_prevote(3, vec![1, 2, 3], 10, 1, new_storage(), false, &l);
...
fn test_learner_respond_vote() -> Result<()> {
    let l = default_logger();
    let mut n1 = new_test_learner_violetabft(1, vec![1, 2], vec![3], 10, 1, new_storage(), &l);
    n1.become_follower(1, INVALID_ID);
    n1.reset_randomized_election_timeout();

    let mut n3 = new_test_learner_violetabft(3, vec![1, 2], vec![3], 10, 1, new_storage(), &l);
    n3.become_follower(1, INVALID_ID);
...
fn test_election_tick_range() {
    ...
    let mut violetabft = new_test_violetabft_with_config(&cfg, s, &l).violetabft.unwrap();
    for _ in 0..1000 {
        violetabft.reset_randomized_election_timeout();
        let randomized_timeout = violetabft.randomized_election_timeout();
        assert!(
    ...
    violetabft = new_test_violetabft_with_config(&cfg, new_storage(), &l)
        .violetabft
        .unwrap();
    for _ in 0..100 {
        violetabft.reset_randomized_election_timeout();
...
fn test_prevote_with_split_vote() {
    ...
    let peers = (1..=3).map(|id| {
        let mut violetabft =
            new_test_violetabft_with_prevote(id, vec![1, 2, 3], 10, 1, new_storage(), true, &l);
        violetabft.become_follower(1, INVALID_ID);
        Some(violetabft)
    });
...
fn test_prevote_with_check_quorum() {
    ...
    let bootstrap = |id| {
        ...
        let s = MemStorage::new_with_conf_state((vec![1, 2, 3], vec![]));
        let mut i = new_test_violetabft_with_config(&cfg, s, &l);
...
// ensure a new VioletaBFT returns a Error::ConfigInvalid with an invalid config
#[test]
fn test_new_violetabft_with_bad_config_errors() {
    let invalid_config = new_test_config(INVALID_ID, 1, 1);
    let s = MemStorage::new_with_conf_state((vec![1, 2], vec![]));
    let violetabft = VioletaBFT::new(&invalid_config, s, &default_logger());
    assert!(violetabft.is_err())
}
...
fn test_batch_msg_append() {
    ...
    let mut violetabft = new_test_violetabft(1, vec![1, 2, 3], 10, 1, storage.clone(), &l);
    violetabft.become_candidate();
    violetabft.become_leader();
    violetabft.set_batch_append(true);
    commit_noop_entry(&mut violetabft, &storage);
    for _ in 0..10 {
        let prop_msg = new_message(1, 1, MessageType::MsgPropose, 1);
        assert!(violetabft.step(prop_msg).is_ok());
    }
    assert_eq!(violetabft.msgs.len(), 2);
    for msg in &violetabft.msgs {
        assert_eq!(msg.entries.len(), 10);
        assert_eq!(msg.index, 1);
    }
    // if the append entry is not continuous, violetabft should not batch the RPC
    let mut reject_msg = new_message(2, 1, MessageType::MsgAppendResponse, 0);
    reject_msg.reject = true;
    reject_msg.index = 2;
    assert!(violetabft.step(reject_msg).is_ok());
    assert_eq!(violetabft.msgs.len(), 3);
}
...
fn test_conf_change_check_before_campaign() {
    ...
    let committed = nt.peers[&2].violetabft_log.committed;
...
fn prepare_request_snapshot() -> (Network, Snapshot) {
    ...
    fn index_term_11(id: u64, ids: Vec<u64>, l: &Logger) -> Interface {
        ...
        store
            ...
            .unwrap();
        let mut violetabft = new_test_violetabft(id, ids, 5, 1, store, &l);
        violetabft.reset(11);
        violetabft
    }
    ...
    assert_eq!(nt.peers[&1].violetabft_log.committed, 14);
    assert_eq!(nt.peers[&2].violetabft_log.committed, 14);
    ...
    nt.peers.get_mut(&1).unwrap().violetabft_log.applied = 14;

    // Commit a new violetabft log.
    let mut test_entries = Entry::default();
    test_entries.data = b"testdata".to_vec();
    let msg = new_message_with_entries(1, 1, MessageType::MsgPropose, vec![test_entries]);
    nt.send(vec![msg]);
...
fn test_follower_request_snapshot() {
    ...
    let request_idx = nt.peers[&1].violetabft_log.committed;
    ...
    assert_eq!(nt.peers[&1].violetabft_log.committed, 16);
    assert_eq!(
        nt.peers[&1].prs().get(2).unwrap().state,
        ProgressState::Snapshot
    );
    assert_eq!(nt.peers[&2].violetabft_log.committed, 15);
    ...
    assert_eq!(nt.peers[&1].violetabft_log.committed, 17);
    assert_eq!(nt.peers[&2].violetabft_log.committed, 17);
}
...
fn test_request_snapshot_matched_change() {
    ...
    nt.peers.get_mut(&2).unwrap().violetabft_log.committed -= 1;

    // Request the latest snapshot.
    let request_idx = nt.peers[&2].violetabft_log.committed;
    nt.peers
        .get_mut(&2)
        .unwrap()
        .request_snapshot(request_idx)
...
fn test_group_commit() {
    ...
    for (i, (matches, group_ids, g_w, q_w)) in tests.drain(..).enumerate() {
        ...
        if sm.violetabft_log.committed != 0 {
            panic!(
                "#{}: follower group committed {}, want 0",
                i, sm.violetabft_log.committed
            );
        }
        sm.state = StateRole::Leader;
        sm.assign_commit_groups(&groups);
        if sm.violetabft_log.committed != g_w {
            panic!(
                "#{}: leader group committed {}, want {}",
                i, sm.violetabft_log.committed, g_w
            );
        }
        sm.enable_group_commit(false);
        if sm.violetabft_log.committed != q_w {
            panic!(
                "#{}: quorum committed {}, want {}",
                i, sm.violetabft_log.committed, q_w
            );
        }
    }
}
...
fn test_election_with_priority_log() {
    ...
    for (_i, &(l1, l2, l3, p1, p2, p3, id, state)) in tests.iter().enumerate() {
        ...
        if l1 {
            n1.violetabft_log.append(&entries);
        }
        if l2 {
            n2.violetabft_log.append(&entries);
        }
        if l3 {
            n3.violetabft_log.append(&entries);
        }
...
fn test_read_when_quorum_becomes_less() {
    ...
    m.set_msg_type(MessageType::MsgHup);
    network.send(vec![m]);
    assert_eq!(network.peers[&1].violetabft_log.committed, 1);
