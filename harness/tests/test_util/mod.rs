...
pub const NOP_STEPPER: Option<Interface> = Some(Interface { violetabft: None });

pub fn ltoa(violetabft_log: &VioletaBFTLog<MemStorage>) -> String {
    let mut s = format!("committed: {}\n", violetabft_log.committed);
    s = s + &format!("applied: {}\n", violetabft_log.applied);
    for (i, e) in violetabft_log.all_entries().iter().enumerate() {
        s = s + &format!("#{}: {:?}\n", i, e);
    }
    s
}
...
pub fn new_test_violetabft(
    ...
    new_test_violetabft_with_config(&config, storage, l)
}

pub fn new_test_violetabft_with_prevote(
    id: u64,
    peers: Vec<u64>,
    election: usize,
    heartbeat: usize,
...
pub fn new_test_violetabft_with_logs(
    id: u64,
    peers: Vec<u64>,
    election: usize,
    heartbeat: usize,
...
pub fn new_test_violetabft_with_config(config: &Config, storage: MemStorage, l: &Logger) -> Interface {
    Interface::new(VioletaBFT::new(config, storage, l).unwrap())
}
...
pub fn soft_state(leader_id: u64, violetabft_state: StateRole) -> SoftState {
    SoftState {
        leader_id,
        violetabft_state,
    }
