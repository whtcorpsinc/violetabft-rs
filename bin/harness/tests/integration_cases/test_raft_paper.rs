...
pub fn commit_noop_entry(r: &mut Interface, s: &MemStorage) {
    ...
    r.read_messages();
    s.wl()
        .append(r.violetabft_log.unstable_entries().unwrap_or(&[]))
        .expect("");
    let committed = r.violetabft_log.committed;
    r.commit_apply(committed);
    let (last_index, last_term) = (r.violetabft_log.last_index(), r.violetabft_log.last_term());
    r.violetabft_log.stable_to(last_index, last_term);
}
...
fn test_start_as_follower() {
    let l = default_logger();
    let r = new_test_violetabft(1, vec![1, 2, 3], 10, 1, new_storage(), &l);
    assert_eq!(r.state, StateRole::Follower);
}
...
fn test_leader_bcast_beat() {
    ...
    // heartbeat interval
    let hi = 1;
    let mut r = new_test_violetabft(1, vec![1, 2, 3], 10, hi, new_storage(), &l);
    r.become_candidate();
    r.become_leader();
    for i in 0..10 {
...
fn test_nonleader_start_election(state: StateRole, l: &Logger) {
    // election timeout
    let et = 10;
    let mut r = new_test_violetabft(1, vec![1, 2, 3], et, 1, new_storage(), l);
    match state {
        StateRole::Follower => r.become_follower(1, 2),
        StateRole::Candidate => r.become_candidate(),
...
fn test_leader_election_in_one_round_rpc() {
    ...
    for (i, (size, votes, state)) in tests.drain(..).enumerate() {
        let mut r = new_test_violetabft(1, (1..=size as u64).collect(), 10, 1, new_storage(), &l);
...
fn test_nonleaders_election_timeout_nonconfict(state: StateRole, l: &Logger) {
    ...
    for id in ids.iter().take(size) {
        rs.push(new_test_violetabft(*id, ids.clone(), et, 1, new_storage(), l));
    }
    let mut conflicts = 0;
    for _ in 0..1000 {
        for r in &mut rs {
        ...
        // several violetabfts time out at the same tick
        if timeout_num > 1 {
            conflicts += 1;
        }
    }
...
fn test_leader_start_replication() {
    let l = default_logger();
    let s = new_storage();
    let mut r = new_test_violetabft(1, vec![1, 2, 3], 10, 1, s.clone(), &l);
    r.become_candidate();
    r.become_leader();
    commit_noop_entry(&mut r, &s);
    let li = r.violetabft_log.last_index();

    r.step(new_message(1, 1, MessageType::MsgPropose, 1))
        .expect("");

    assert_eq!(r.violetabft_log.last_index(), li + 1);
    assert_eq!(r.violetabft_log.committed, li);
    let mut msgs = r.read_messages();
    msgs.sort_by_key(|m| format!("{:?}", m));
    ...
    assert_eq!(msgs, expect_msgs);
    assert_eq!(r.violetabft_log.unstable_entries(), Some(&*wents));
}
...
fn test_leader_commit_entry() {
    ...
    assert_eq!(r.violetabft_log.committed, li + 1);
    let wents = vec![new_entry(1, li + 1, SOME_DATA)];
    assert_eq!(r.violetabft_log.next_entries(), Some(wents));
    let mut msgs = r.read_messages();
    msgs.sort_by_key(|m| format!("{:?}", m));
...
fn test_leader_acknowledge_commit() {
    ...
    for (i, (size, acceptors, wack)) in tests.drain(..).enumerate() {
        let s = new_storage();
        let mut r = new_test_violetabft(1, (1..=size).collect(), 10, 1, s.clone(), &l);
        r.become_candidate();
        r.become_leader();
        commit_noop_entry(&mut r, &s);
        let li = r.violetabft_log.last_index();
        ...
        let g = r.violetabft_log.committed > li;
        if g ^ wack {
...
fn test_leader_commit_preceding_entries() {
    ...
    for (i, mut tt) in tests.drain(..).enumerate() {
        let mut r = {
            ...
            new_test_violetabft_with_config(&cfg, store, &l)
        };
        ...
        let g = r.violetabft_log.next_entries();
        let wg = Some(tt);
        if g != wg {
...
fn test_follower_commit_entry() {
    ...
    for (i, (ents, commit)) in tests.drain(..).enumerate() {
        ...
        if r.violetabft_log.committed != commit {
            panic!(
                "#{}: committed = {}, want {}",
                i, r.violetabft_log.committed, commit
            );
        }
...
fn test_follower_append_entries() {
    ...
    for (i, (index, term, ents, wents, wunstable)) in tests.drain(..).enumerate() {
        ...
        let g = r.violetabft_log.all_entries();
        if g != wents {
            panic!("#{}: ents = {:?}, want {:?}", i, g, wents);
        }
        let g = r.violetabft_log.unstable_entries();
...
fn test_leader_sync_follower_log() {
    ...
    for (i, tt) in tests.drain(..).enumerate() {
        ...
        let last_index = lead.violetabft_log.last_index();
        ...
        n.send(vec![m]);
        let lead_str = ltoa(&n.peers[&1].violetabft_log);
        let follower_str = ltoa(&n.peers[&2].violetabft_log);
...
fn test_leader_only_commits_log_from_current_term() {
    ...
    for (i, (index, wcommit)) in tests.drain(..).enumerate() {
        ...
        m.index = index;
        r.step(m).expect("");
        if r.violetabft_log.committed != wcommit {
            panic!(
                "#{}: commit = {}, want {}",
                i, r.violetabft_log.committed, wcommit
            );
        }
    }
}
