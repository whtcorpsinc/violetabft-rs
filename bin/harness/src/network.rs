...
use violetabft::{
    evioletabftpb::{ConfState, Message, MessageType},
    storage::MemStorage,
    Config, VioletaBFT, Result, NO_LIMIT,
};
use slog::Logger;
...
///
/// You can use this to create a test network of VioletaBFT nodes.
///
/// *Please note:* no actual network calls are made.
#[derive(Default)]
pub struct Network {
    /// The set of violetabft peers.
    pub peers: HashMap<u64, Interface>,
    /// The storage of the violetabft peers.
...
impl Network {
    ...
    ///
    /// A `None` node will be replaced with a new VioletaBFT node, and its configuration will
    /// be `peers`.
        ...
        for (p, id) in peers.drain(..).zip(&peer_addrs) {
            match p {
                None => {
                    ...
                    config.id = *id;
                    let r = VioletaBFT::new(&config, store, l).unwrap().into();
                    npeers.insert(*id, r);
                }
                Some(r) => {
                    if let Some(violetabft) = r.violetabft.as_ref() {
                        if violetabft.id != *id {
                            panic!("peer {} in peers has a wrong position", r.id);
                        }
                        let store = violetabft.violetabft_log.store.clone();
    ...
    /// Isolate the given violetabft to and from all other violetabft in the cluster.
