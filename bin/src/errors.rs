...
quick_error! {
    /// The base error type for violetabft
    #[derive(Debug)]
    pub enum Error {
        ...
        /// VioletaBFT cannot step the local message.
        StepLocalMsg {
            description("violetabft: cannot step violetabft local message")
        }
        /// The violetabft peer is not found and thus cannot step.
        StepPeerNotFound {
            description("violetabft: cannot step as peer not found")
        }
        /// The proposal of changes was dropped.
        ProposalDropped {
            description("violetabft: proposal dropped")
        }
        ...
        RequestSnapshotDropped {
            description("violetabft: request snapshot dropped")
        }
    }
}
...
/// A result type that wraps up the violetabft errors.
