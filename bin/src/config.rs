...
/// Config contains the parameters to start a violetabft.
#[derive(Clone)]
pub struct Config {
    /// The identity of the local violetabft. It cannot be 0, and must be unique in the group.
    pub id: u64,
    ...
    /// violetabft. violetabft will not return entries to the application smaller or equal to Applied.
    /// If Applied is unset when restarting, violetabft might return previous applied entries.
    /// This is a very application dependent configuration.
    pub applied: u64,

    /// Limit the max size of each append message. Smaller value lowers
    /// the violetabft recovery cost(initial probing and message lost during normal operation).
    ...
    /// Enables the Pre-Vote algorithm described in violetabft thesis section
    ...
    /// Don't broadcast an empty violetabft entry to notify follower to commit an entry.
