...
use violetabft::{prelude::*, StateRole};
use regex::Regex;
...
fn main() {
    ...
    // after it's committed by the violetabft cluster, it will be poped from the queue.
    ...
    for (i, rx) in rx_vec.into_iter().enumerate() {
        ...
        let mut node = match i {
            ...
            0 => Node::create_violetabft_leader(1, rx, mailboxes, &logger),
            // Other peers are followers.
            _ => Node::create_violetabft_follower(rx, mailboxes),
        };
        let proposals = Arc::clone(&proposals);

        // Tick the violetabft node per 100ms. So use an `Instant` to trace it.
        ...
        let handle = thread::spawn(move || loop {
            ...
            let violetabft_group = match node.violetabft_group {
                Some(ref mut r) => r,
                // When Node::violetabft_group is `None` it means the node is not initialized.
                _ => continue,
            };

            if t.elapsed() >= Duration::from_millis(100) {
                // Tick the violetabft.
                violetabft_group.tick();
                t = Instant::now();
            }

            // Let the leader pick pending proposals from the global queue.
            if violetabft_group.violetabft.state == StateRole::Leader {
                // Handle new proposals.
                let mut proposals = proposals.lock().unwrap();
                for p in proposals.iter_mut().skip_while(|p| p.proposed > 0) {
                    propose(violetabft_group, p);
                }
            }

            // Handle readies from the violetabft.
            on_ready(
                violetabft_group,
                &mut node.kv_pairs,
                &node.mailboxes,
    ...
    info!(
        logger,
        "We get a 5 nodes VioletaBFT cluster now, now propose 100 proposals"
    );
    (0..100u16)
        .filter(|i| {
...
struct Node {
    // None if the violetabft is not initialized.
    violetabft_group: Option<RawNode<MemStorage>>,
    my_mailbox: Receiver<Message>,
    mailboxes: HashMap<u64, Sender<Message>>,
    // Key-value pairs after applied. `MemStorage` only contains violetabft logs,
    // so we need an additional storage engine.
    kv_pairs: HashMap<u16, String>,
}

impl Node {
    // Create a violetabft leader only with itself in its configuration.
    fn create_violetabft_leader(
        id: u64,
        my_mailbox: Receiver<Message>,
        mailboxes: HashMap<u64, Sender<Message>>,
        logger: &slog::Logger,
        ...
        let violetabft_group = Some(RawNode::new(&cfg, storage, &logger).unwrap());
        Node {
            violetabft_group,
            my_mailbox,
            mailboxes,
    ...
    // Create a violetabft follower.
    fn create_violetabft_follower(
        my_mailbox: Receiver<Message>,
        mailboxes: HashMap<u64, Sender<Message>>,
    ) -> Self {
        Node {
            violetabft_group: None,
            my_mailbox,
            mailboxes,
            kv_pairs: Default::default(),
        }
    ...
    // Initialize violetabft for followers.
    fn initialize_violetabft_from_message(&mut self, msg: &Message, logger: &slog::Logger) {
        ...
        let storage = MemStorage::new();
        self.violetabft_group = Some(RawNode::new(&cfg, storage, &logger).unwrap());
    }

    // Step a violetabft message, initialize the violetabft if need.
    fn step(&mut self, msg: Message, logger: &slog::Logger) {
        if self.violetabft_group.is_none() {
            if is_initial_msg(&msg) {
                self.initialize_violetabft_from_message(&msg, &logger);
            } else {
                return;
            }
        }
        let violetabft_group = self.violetabft_group.as_mut().unwrap();
        let _ = violetabft_group.step(msg);
    }
}

fn on_ready(
    violetabft_group: &mut RawNode<MemStorage>,
    kv_pairs: &mut HashMap<u16, String>,
    mailboxes: &HashMap<u64, Sender<Message>>,
    proposals: &Mutex<VecDeque<Proposal>>,
    logger: &slog::Logger,
) {
    if !violetabft_group.has_ready() {
        return;
    }
    let store = violetabft_group.violetabft.violetabft_log.store.clone();

    // Get the `Ready` with `RawNode::ready` interface.
    let mut ready = violetabft_group.ready();

    // Persistent violetabft logs. It's necessary because in `RawNode::advance` we stabilize
    // violetabft logs to the latest position.
    if let Err(e) = store.wl().append(ready.entries()) {
        error!(
            logger,
            "persist violetabft log fail: {:?}, need to retry or panic", e
        );
        return;
    }
    ...
    for msg in ready.messages.drain(..) {
        ...
        if mailboxes[&to].send(msg).is_err() {
            error!(
                logger,
                "send violetabft message to {} fail, let VioletaBFT retry it", to
            );
        }
    }
    ...
    if let Some(committed_entries) = ready.committed_entries.take() {
        for entry in &committed_entries {
            ...
            if let EntryType::EntryConfChange = entry.get_entry_type() {
                ...
                let cs = violetabft_group.apply_conf_change(&cc).unwrap();
    ...
    // Call `RawNode::advance` interface to update position flags in the violetabft.
    violetabft_group.advance(ready);
}
...
// The message can be used to initialize a violetabft node or not.
...
fn propose(violetabft_group: &mut RawNode<MemStorage>, proposal: &mut Proposal) {
    let last_index1 = violetabft_group.violetabft.violetabft_log.last_index() + 1;
    if let Some((ref key, ref value)) = proposal.normal {
        let data = format!("put {} {}", key, value).into_bytes();
        let _ = violetabft_group.propose(vec![], data);
    } else if let Some(ref cc) = proposal.conf_change {
        let _ = violetabft_group.propose_conf_change(vec![], cc.clone());
    } else if let Some(_transferee) = proposal.transfer_leader {
        // TODO: implement transfer leader.
        unimplemented!();
    }

    let last_index2 = violetabft_group.violetabft.violetabft_log.last_index() + 1;
